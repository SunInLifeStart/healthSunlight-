...
const specialItem = {
  ...
  mutations: {
    [SET_TREETABLE]: (state, data) => {
      ...
      calculateCost(data.items)
      state.treeTable = data
    },
    [SAVE_SPECIALITEM]: (state, data) => {
      state.treeTable.items = data
    }
  },
  actions: {
    // 获取各个项目信息
    GetSpecialItem({
      ...
      return getTreeTable(ids).then(result => {
        commit(SET_TREETABLE, result.data)
        return result
      })
    },
    // 页面的元素发生变化时调用
    ...
    SaveSpecialItem({
      ...
      commit(SAVE_SPECIALITEM, items)
      return saveSpecialItem(items).then(result => {
        return result
      })
    }
  }
